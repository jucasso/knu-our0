function designerProfile(designer) {
  return `<a href="/exhibition/a_designer.html?designer=${encodeURIComponent(designer.name_en)}">
        ...
        <div class="img-box">
            <img src="${designer.identity_img}" alt="${designer.name_kr}" style="background: white;">
        </div>
    </a>`;
}
...
fetch("../public/test.json")
  ...
  .then((data) => {
    ...
    for (const student of data) {
      const p = student.projects.find((p) => p.project_name == project_name);
      if (p) {
        tmp.project = tmp.project ?? p;
        tmp.designers.push(student);
      }
    }
    console.log(tmp);
